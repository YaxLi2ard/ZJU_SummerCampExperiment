...
def main(idx):
    ...
    #     embed_dim=[64, 128, 256, 320],  # 128, 256, 512, 640
    #     num_heads=8,
    #     addition=True,
    #     mlp_drop_rate=0.0,
    #     head_drop_rate=0.0,
    ...
    model = Spiking_vit_MetaFormer_lite(
        ...
        num_heads=8,
        addition=True,
        mlp_drop_rate=config_dic['dropout_mlp'],
        head_drop_rate=config_dic['dropout_head'],
        mlp_ratios=4,
        in_channels=2,
        num_classes=11,
    )
    ...
    checkpoint = torch.load('Spike-DrivenV2_last2.pkl', map_location=device)
    ...
    elif config_dic['optim'] == 'AdamW':
        optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=config_dic['weight_decay'])
    else:
...
if __name__ == '__main__':
    ...
    hp_config = [
        {},
        # 1
        {'lr': 1e-4, 'batch_size': 9, 'epochs': 30, 'loss': 'CE', 'dropout_mlp': 0.2, 'dropout_head': 0.1, 'optim': 'AdamW', 'weight_decay': 0.0005},
        # 2
        {'lr': 1e-4, 'batch_size': 9, 'epochs': 30, 'loss': 'CE', 'dropout_mlp': 0.1, 'dropout_head': 0.1, 'optim': 'Adam', 'weight_decay': 0.0005},
        # 3
        {'lr': 1e-4, 'batch_size': 9, 'epochs': 30, 'loss': 'CE', 'dropout_mlp': 0.1, 'dropout_head': 0.1, 'optim': 'AdamW', 'weight_decay': 0.0005},
        # 4
        {'lr': 1e-4, 'batch_size': 6, 'epochs': 30, 'loss': 'CE', 'dropout_mlp': 0.1, 'dropout_head': 0.1, 'optim': 'AdamW', 'weight_decay': 0.0005},
        # 5
        {'lr': 1e-4, 'batch_size': 9, 'epochs': 30, 'loss': 'CE', 'dropout_mlp': 0.2, 'dropout_head': 0.1, 'optim': 'Adam', 'weight_decay': 0.0005},
        # 6
        {'lr': 1e-4, 'batch_size': 9, 'epochs': 30, 'loss': 'CE', 'dropout_mlp': 0.1, 'dropout_head': 0.1, 'optim': 'AdamW', 'weight_decay': 0.005},
        # 7
        {'lr': 1e-4, 'batch_size': 9, 'epochs': 30, 'loss': 'CE', 'dropout_mlp': 0.3, 'dropout_head': 0.3, 'optim': 'AdamW', 'weight_decay': 0.0005},
        # 8
        {'lr': 1e-4, 'batch_size': 11, 'epochs': 30, 'loss': 'CE', 'dropout': 0.1, 'optim': 'Adam', 'weight_decay': 0.000},
    ]
