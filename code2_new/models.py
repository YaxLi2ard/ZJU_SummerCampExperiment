...
class Spiking_vit_MetaFormer_lite(nn.Module):
    def __init__(
        self,
        in_channels=2,
        num_classes=11,
        ...
        # self.tb = TimeBlock(in_c=embed_dim[1])
...
#         embed_dim=[64, 128, 256, 320],  # 96, 192, 384, 480
#         num_heads=8,
#         addition=True,
#         mlp_drop_rate=0.1,
#         head_drop_rate=0.1,
...
# y = model(x)
# print(y.shape)
# torch.save(model.state_dict(), 'ttt.pkl')

# class TimeBlock(nn.Module):
#     def __init__(self, in_c=256, time_c=8):
#         super(TimeBlock, self).__init__()
#         self.conv3d = nn.Conv3d(in_channels=in_c, out_channels=in_c, kernel_size=3, stride=1, padding=1)
#         self.conv1d = nn.Conv1d(in_channels=in_c, out_channels=in_c, kernel_size=3, stride=1, padding=1)
#         self.conv2d = nn.Conv2d(in_channels=in_c, out_channels=in_c, kernel_size=3, stride=1, padding=1)
#         self.lif = MultiStepLIFNode(tau=2.0, detach_reset=True)
...
#         ide = x
#         # c1 = self.conv3d(x)
#         x = x.permute(0, 3, 4, 1, 2).flatten(0, 2)
#         x = self.conv1d(x)
#         x = x.reshape(B, H, W, C, T).permute(0, 4, 3, 1, 2).flatten(0, 1)
#         x = self.conv2d(x)
#         c2 = x.reshape(B, T, C, H, W).permute(0, 2, 1, 3, 4)
#         # y = ide + c1 + c2
#         y = ide + c2
#         y = self.lif(y)
#         return y
...
class TimeBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 4, 1, 2).flatten(0, 2)
        x = self.conv1d(x)
        c1 = x.reshape(B, H, W, C, T).permute(0, 3, 4, 1, 2)
        y = self.lif(c1) + ide
        return y

class Spiking_vit_MetaFormer_lite2(nn.Module):
    def __init__(
        self,
        in_channels=2,
        num_classes=11,
        ...
        self.tb1 = TimeBlock(in_c=embed_dim[0] // 2)
        ...
        self.tb2 = TimeBlock(in_c=embed_dim[0])
    ...
    def forward_features(self, x):
        ...
        x = self.tb1(x.permute(1, 2, 0, 3, 4))
        x = x.permute(2, 0, 1, 3, 4)
        ...
        x = self.tb2(x.permute(1, 2, 0, 3, 4))
        x = x.permute(2, 0, 1, 3, 4)
