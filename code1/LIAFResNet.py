...
class LIAFResNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.td = nn.Linear(1,1)
        self.fc = nn.Sequential(
            nn.Linear(self.cfgFc_[0],self.cfgFc_[0]),
            nn.Dropout(p=config.dp),
            nn.Linear(self.cfgFc_[0],self.cfgFc_[1]),
        )
    ...
    def forward(self,input):
        ...
        self.device  = self.fc[0].weight.device
